...
const deleteTeam = async (req, res) => {
  try {
    ...
    if (!findTeam) {
      return res.status(400).json({ message: "Team not found to delete" });
    }

    const tournamentId = findTeam?.tournamentId;

    const deleteTeam = await Team.findByIdAndDelete(id);

    const findTeams = await Team.find({ tournamentId });

    if (!findTeams) {
      return res.status(400).json({ message: "no teams found" });
    }

    const deleteRounds = await Round.deleteMany({ tournamentId });
    const deleteMatchs = await Match.deleteMany({ tournamentId });
    ...
    try {
      ...
      let findTeams = await Team.find({ tournamentId: savedTournament._id });

      // Apply fixing type
      if (savedTournament.fixingType === "RANDOM") {
        findTeams = shuffleTeams(findTeams);
      } else if (savedTournament.fixingType === "TOP_VS_BOTTOM") {
      ...
      for (let round of findAllRounds) {
        ...
        for (let i = 0; i < roundMatches.length; i += 2) {
          if (roundMatchIdsMap.get(round.roundNumber + 1)) {
            let nextRoundMatchesIds = roundMatchIdsMap.get(
              round.roundNumber + 1
            );
      ...
      return res.status(400).json({ message: "unable to create tournament" });
    }
